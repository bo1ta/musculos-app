...
struct ExerciseFactory: ExerciseProvider {
  var isFavorite: Bool
  var name: String
  var category: String
  var level: String
  ...
  static func create(name: String = "Power Stairs",
  ...
  ["In the power stairs, implements are moved up a staircase. For training purposes, these can be performed with a tire or box.",
    ...
    "Repeat for 3-5 repetitions, and continue with a heavier weight, moving as fast as possible."]
      ...
      return self.init(isFavorite: true, name: name, category: category, level: level, instructions: instructions, imageUrls: [], primaryMuscles: primaryMuscles, secondaryMuscles: secondaryMuscles)
    }
}
